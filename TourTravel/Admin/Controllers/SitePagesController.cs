  ...
  public class SitePagesController : Controller
  {
    ...
    public async Task<IActionResult> Create(SitePages model, IFormFile? ImageFile)
    {
      ...
      if (ModelState.IsValid)
      {
        ...
        {
          bool TitleExists = await _db.SitePages.AnyAsync(t => t.Page == model.Page);
          if (TitleExists)
          {
            return Json(new { success = false, message = "Page already exists. Cannot add duplicate." });
    ...
    public async Task<IActionResult> Edit(int id, SitePages model, IFormFile? ImageFile)
    {
      ...
      if (ModelState.IsValid)
      {
        bool TitleExists = await _db.SitePages.AnyAsync(t => t.Page.Trim().ToLower() == model.Page.Trim().ToLower() && t.Id != model.Id);
        if (TitleExists)
        {
          return Json(new { success = false, message = "Page already exists. Cannot Update duplicate." });
    ...
    public async Task<IActionResult> Delete(int id)
    {
      ...
      return Json(new { success = true, message = "Site Page deleted successfully." });
    }
  }
}
